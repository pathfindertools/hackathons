...
export const Modals = ({ data, parentField = "" }) => {
  ...
  return (
    <Section background={data.background} navigationLabel={data.navigationLabel}>
      <div className={`relative flex flex-col w-full max-w-site-full mx-auto ${data.style?.padding}`}>
        <div className={`flex-none ${data.style?.featureContent}`}>
         <Content
            data = {data}
            styles = {data.style}
            alignment = {`${data.textAlign} ${data.textAlignMobile}`}
            width = "w-full"
...
export const modalsBlockSchema: any = {
  name: "modals",
  label: "Video Modals",
  ui: {
    defaultItem: {
